...
config.plugins.iptvplayer.swatchseries_web_proxy_gateway_provider = ConfigSelection(default = "hideproxymeus", choices = [("hideproxymenl", _("hideproxy.me (NL)")),("hideproxymeus", _("hideproxy.me (US)")),("hideproxymede", _("hideproxy.me (DE)"))])

def GetConfigList():
    optionList = []
    optionList.append(getConfigListEntry(_("Use web proxy gateway"), config.plugins.iptvplayer.swatchseries_web_proxy_gateway))
    optionList.append(getConfigListEntry(_("Web proxy provider"), config.plugins.iptvplayer.swatchseries_web_proxy_gateway_provider))
    return optionList
###################################################
...
class TheWatchseriesTo(CBaseHostClass):
    ...
    def getProxyProvider(self):
        # provides support for end user to select an alternative
        # proxy from the host configuration -i'll be adding more
        # proxies in the next update of this host.  CM
        val = config.plugins.iptvplayer.swatchseries_web_proxy_gateway_provider.value
        if val == 'hideproxymenl':
            return 'https://nl.hideproxy.me/go.php?u={0}&b=4'
        elif val == 'hideproxymeus':
            return 'https://us.hideproxy.me/go.php?u={0}&b=4'
        elif val == 'hideproxymede':
            return 'https://de.hideproxy.me/go.php?u={0}&b=4'
        else:
            return 'https://nl.hideproxy.me/go.php?u={0}&b=4'
